...
# get OnPrem Instance Id
$instanceid = cmd /c "`"${Env:ProgramFiles}\Amazon\SSM\ssm-cli.exe`" get-instance-information"
$instance = $instanceid.split('"')[3];
...
Function CWAConfig() {
    ...
    if ($jsonDirContent.count -eq 0) {
        ...
        #### MSP Modification for OnPrem Instances only to support native AWS dynamic InstanceId insertion format
        $targetfile = Get-ChildItem "${JSON_DIR}\" | Select FullName
        Write-Output "---- GDIT Hive on AWS updates for dynamic Instance Id insertion support for OnPremise Instance ------" 
        #$current = Get-Content -Path "${JSON_DIR}\"
        $filename = $ConfigLocation.Replace('ssm:','')
        $targetpath = $JSON_DIR + '\' + 'ssm_' + $filename + '.tmp'
        #Write-Output "current var values: ${TOML} ${JSON_DIR} ${ConfigLocation} ${COMMON_CONIG} ${multi_config}"
        Write-Output "Updating for OnPrem Instance Id:"  $instance
        #((Get-Content -path $TOML) -replace "${aws:InstanceId}","$instance") | Set-Content -path $TOML
        $current = Get-Content -Path $targetpath
        #Write-Output "current file:" $current
        $result = $current.Replace('${aws:InstanceId}', $instance)
        Set-Content -Path $targetpath -Value $result -Force
        #### End MSP modifications
