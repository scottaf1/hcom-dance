...
# HCOM updates line 73-74, 276 - 289. Last modified 7/25/2023 tom.moore@gdit.com
set -e
set -u
...
cwa_config() {
     ...
     if [ ! "$(ls ${JSON_DIR})" ]; then
          ...
          ## HCOM Custom Begins
          echo "config_location: " ${cwa_config_location}
          filename=${cwa_config_location}
          echo "--- filename " ${filename}
          filename=${filename/ssm:/}
          echo "--- filename mod" ${filename}
          targetpath="/opt/aws/amazon-cloudwatch-agent/etc/amazon-cloudwatch-agent.d/ssm_${filename}.tmp"
          echo "--- targetpath" ${targetpath}
          current=$(cat ${targetpath})
          #echo "--- current json " ${current}
          result=${current//'${aws:InstanceId}'/$instanceid}
          echo "--- final json " ${result}
          echo ${result} > ${targetpath}
          ## HCOM Custom End
